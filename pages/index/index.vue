		...
		<view class="bookList flex align-center justify-center" v-if="popularList.length>0">
			<view class="bookList-box">
				...
				<view class="bookList-box-title flex align-center justify-between">
					...
					<view @click="gotoNav('/pages/moreBook/moreBook?cate=1')"
		...
		<view class="bookList flex align-center justify-center" v-if="newList.length>0">
			<view class="bookList-box">
				...
				<view class="bookList-box-title flex align-center justify-between">
					...
					<view @click="gotoNav('/pages/moreBook/moreBook?cate=2')"
						class="bookList-box-title-r flex align-center">
						更多
						<u-icon name="arrow-right" color="#999999" size="14"></u-icon>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			//获取背景图
			getBgImg() {
				this.$Request.get('/app/banner/selectBannerList?classify=5').then(res => {
					if (res.code == 0) {
			...
			// 获取书籍
			getPopularList(cate, limit) {
				let data = {
					limit: limit,
					page: this.page,
					cate: cate,
				}
				this.$Request.get('/app/course/selectCourse', data).then(res => {
					if (res.code == 0) {

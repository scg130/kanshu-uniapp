		...
		<view class="info flex align-center justify-center">
			<view class="info-box">
				...
				<view class="info-box-mum flex align-center justify-between">
					...
					<view style="margin-right: 10rpx;" class="info-box-mum-item" @click="notOpen()">
		...
		<view class="money flex align-center justify-center">
			<view class="money-box">
				...
				<view class="money-box-btn flex align-center justify-between">
					...
					<view class="money-box-btn-l" @click="notOpen()">
						<image src="../../static/image/my/qrCode.png" mode=""></image>
					</view>
					<view class="money-box-btn-r flex align-center">
						<view style="display: none;" @click="goNav('/package/wallet/wallet')"
		...
		<view class="tool flex align-center justify-center">
			<view class="tool-box">
				...
				<view class="tool-box-content flex align-center justify-between flex-wrap">
					...
					<view class="tool-box-content-item flex align-center justify-center flex-wrap" @click="goKefuMsg()">
						<view class="tool-box-content-item-img">
				...
				this.$Request.get('/app/user/selectUserById?user_id='+uni.getStorageSync('userId')).then(res => {
					if (res.code == 0) {
						if (res.data.is_vip == 1) {
							this.isVIP = true;
							this.endTime = res.data.vip_end_time
						...
						this.jifen = res.data.integral_num
						this.myLoveNum = res.data.love_num
						this.myZhui = res.data.note_num
						this.userName = res.data.username
						if (res.data.avatar != "") {
							this.avatar = res.data.avatar
						} else {
							this.avatar = '../../static/avatar.jpg'
						}
						let money = res.data.invite_money+res.data.money
						this.moneyNum = money.toFixed(2)
						this.invitationCode = res.data.invitation_code
						uni.setStorageSync('userName', res.data.username)
						uni.setStorageSync('avatar', this.avatar)
						uni.setStorageSync('invitationCode', res.data.invitation_code)
					} else {
						this.goLogin()
					}
				})
